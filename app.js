...
function dashboardFinanceSummary(){
  ...
  if(!s.count&&!state.financeFinancingPlans.length)return `<section class="card dashboard-finance"><div class="row"><h2 style="flex:1">Finans Özeti</h2><button onclick="page('finance')">Finans’a Git</button></div><p class="muted">Finans özeti için önce hesap veya finansman kaydı ekleyin.</p></section>`;
...
function financeFinancingSummary(plan){const principal=Number(plan.principal_amount||0),commission=Number(plan.commission_amount||0),installmentSummary=financeInstallmentSummary(plan.id),hasInstallments=installmentSummary.count>0,baseTotalMonths=Math.max(1,Number(plan.total_months||1)),totalMonths=hasInstallments?installmentSummary.count:baseTotalMonths,paidMonths=hasInstallments?installmentSummary.paidCount:Math.min(baseTotalMonths,Math.max(0,Number(plan.paid_months||0))),monthlyPayment=Number(plan.monthly_payment||0)||principal/baseTotalMonths,totalScheduled=hasInstallments?installmentSummary.total:monthlyPayment*totalMonths,paidAmount=hasInstallments?installmentSummary.paid:monthlyPayment*paidMonths,remainingMonths=hasInstallments?Math.max(0,installmentSummary.count-installmentSummary.paidCount):totalMonths-paidMonths,remainingAmount=Math.max(0,totalScheduled-paidAmount),totalCost=principal+commission,progressPercent=totalMonths?(paidMonths/totalMonths)*100:0;return {principal,commission,totalMonths,paidMonths,monthlyPayment,remainingMonths,paidAmount,remainingAmount,totalCost,progressPercent}}
...
async function saveFinanceFinancingInstallment(planId,id=''){const row={user_id:user.id,financing_plan_id:planId,installment_no:Number($('fi_installment_no').value)||1,due_date:$('fi_due_date').value,amount:Number($('fi_amount').value)||0,paid_amount:Number($('fi_paid_amount').value)||0,paid_date:$('fi_paid_date').value||null,status:$('fi_status').value,notes:$('fi_notes').value.trim()||null};if(row.status==='paid'&&row.paid_amount===0)row.paid_amount=row.amount;if(row.status==='paid'&&!row.paid_date)row.paid_date=today();if(['pending','overdue','cancelled'].includes(row.status))row.paid_amount=0;if(!row.financing_plan_id)return toast('Finansman seçimi gerekli.');if(row.installment_no<=0)return toast('Taksit no 0’dan büyük olmalı.');if(!row.due_date)return toast('Vade tarihi gerekli.');if(row.amount<0||row.paid_amount<0)return toast('Tutarlar negatif olamaz.');if(row.paid_amount>row.amount&&row.status!=='paid')return toast('Ödenen tutar taksit tutarından büyükse durum Ödendi olmalı.');const q=id?sb.from('finance_financing_installments').update(row).eq('id',id).eq('user_id',user.id):sb.from('finance_financing_installments').insert(row);const {error}=await q;if(error)return toast(error.message);const nextRows=state.financeFinancingInstallments.filter(i=>i.financing_plan_id===planId&&i.id!==id).concat({...row,id:id||'new'});const paid_months=nextRows.filter(i=>i.status==='paid').length;await sb.from('finance_financing_plans').update({paid_months}).eq('id',planId).eq('user_id',user.id);closeModal();toast('Taksit kaydedildi.');await load();page('finance')}
...
function financeReportRange(){const month=currentBudgetMonth();return {from:$('finance_report_from')?.value||`${month}-01`,to:$('finance_report_to')?.value||`${month}-31`}}
function financeReportInRange(date,range){return !!date&&date>=range.from&&date<=range.to}
function financeReportMonthKey(date){return (date||'').slice(0,7)||'Tarihsiz'}
function financeReportsData(range=financeReportRange()){const tx=state.financeTransactions.filter(t=>(t.status||'completed')==='completed'&&financeReportInRange(t.transaction_date,range)&&(t.currency||'TRY').toUpperCase()==='TRY');const monthly={};tx.forEach(t=>{const m=financeReportMonthKey(t.transaction_date);monthly[m]??={month:m,income:0,expense:0,net:0};monthly[m][t.transaction_type==='income'?'income':'expense']+=Number(t.amount||0);monthly[m].net=monthly[m].income-monthly[m].expense});const categories={};tx.filter(t=>t.transaction_type==='expense').forEach(t=>{const k=t.title||'Diğer';categories[k]??={category:k,amount:0,count:0};categories[k].amount+=Number(t.amount||0);categories[k].count++});const financing=state.financeFinancingInstallments.filter(i=>i.status==='paid'&&financeReportInRange(i.paid_date||i.due_date,range)).map(i=>{const plan=state.financeFinancingPlans.find(p=>p.id===i.financing_plan_id);return {provider:plan?.provider_name||'Finansman',installment_no:Number(i.installment_no||0),date:i.paid_date||i.due_date,amount:Number(i.paid_amount||i.amount||0)}});const budgetMonths=[...new Set(state.financeBudgets.map(b=>b.month).filter(m=>m&&(`${m}-01`<=range.to)&&(`${m}-31`>=range.from)))];const budgets=budgetMonths.flatMap(month=>financeBudgetsForMonth(month).map(b=>{const p=financeBudgetProgress(b);return {month,category:b.category,budget:p.amount,actual:p.actual,remaining:p.remaining,percent:p.percent}}));return {monthly:Object.values(monthly).sort((a,b)=>a.month.localeCompare(b.month)),categories:Object.values(categories).sort((a,b)=>b.amount-a.amount),financing,budgets}}
function csvCell(v){const s=String(v??'');return /[",\n;]/.test(s)?`"${s.replaceAll('"','""')}"`:s}
function exportFinanceReportCsv(type='monthly'){const d=financeReportsData(),rows={monthly:[['Ay','Gelir','Gider','Net'],...d.monthly.map(r=>[r.month,r.income,r.expense,r.net])],categories:[['Kategori','Tutar','Adet'],...d.categories.map(r=>[r.category,r.amount,r.count])],financing:[['Kurum','Taksit No','Tarih','Tutar'],...d.financing.map(r=>[r.provider,r.installment_no,r.date,r.amount])],budgets:[['Ay','Kategori','Bütçe','Gerçekleşen','Kalan','%'],...d.budgets.map(r=>[r.month,r.category,r.budget,r.actual,r.remaining,r.percent])]}[type]||[];const csv=rows.map(r=>r.map(csvCell).join(';')).join('\n');const blob=new Blob([csv],{type:'text/csv;charset=utf-8'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`momentum-finance-${type}-report.csv`;a.click()}
function financeReportRows(rows,empty,mapper){return rows.length?rows.map(mapper).join(''):`<div class="empty compact">${empty}</div>`}
function financeReportsPanel(){const r=financeReportRange(),d=financeReportsData(r);return `<section id="finance_reports_panel" class="card"><div class="row"><div style="flex:1"><span class="eyebrow">Finance Reports</span><h2>Finans Raporları</h2><p class="muted">Tarih aralığına göre gelir/gider, kategori, finansman ve bütçe gerçekleşen özetleri.</p></div></div><div class="split"><div><label>Başlangıç</label><input id="finance_report_from" type="date" value="${esc(r.from)}" onchange="page('finance')"></div><div><label>Bitiş</label><input id="finance_report_to" type="date" value="${esc(r.to)}" onchange="page('finance')"></div></div><div class="split finance-report-grid"><article><div class="row"><h3 style="flex:1">Aylık Gelir/Gider Raporu</h3><button class="small secondary" onclick="exportFinanceReportCsv('monthly')">CSV</button></div>${financeReportRows(d.monthly,'Seçilen aralıkta hareket yok.',x=>`<div class="pill inactive"><span>${esc(x.month)} · Gelir ${financeFmt(x.income,'TRY')} · Gider ${financeFmt(x.expense,'TRY')}</span><b class="${x.net>=0?'pos':'neg'}">${financeFmt(x.net,'TRY')}</b></div>`)}</article><article><div class="row"><h3 style="flex:1">Kategori Bazlı Gider</h3><button class="small secondary" onclick="exportFinanceReportCsv('categories')">CSV</button></div>${financeReportRows(d.categories,'Seçilen aralıkta gider yok.',x=>`<div class="pill inactive"><span>${esc(x.category)} · ${x.count} kayıt</span><b class="neg">${financeFmt(x.amount,'TRY')}</b></div>`)}</article><article><div class="row"><h3 style="flex:1">Finansman/Kredi Ödeme Raporu</h3><button class="small secondary" onclick="exportFinanceReportCsv('financing')">CSV</button></div>${financeReportRows(d.financing,'Seçilen aralıkta ödenmiş taksit yok.',x=>`<div class="pill inactive"><span>${esc(x.provider)} #${x.installment_no} · ${esc(x.date)}</span><b class="pos">${financeFmt(x.amount,'TRY')}</b></div>`)}</article><article><div class="row"><h3 style="flex:1">Bütçe / Gerçekleşen</h3><button class="small secondary" onclick="exportFinanceReportCsv('budgets')">CSV</button></div>${financeReportRows(d.budgets,'Seçilen aralıkta bütçe yok.',x=>`<div class="pill inactive"><span>${esc(x.month)} · ${esc(x.category)} · %${x.percent}</span><b class="${x.remaining>=0?'pos':'neg'}">${financeFmt(x.actual,'TRY')} / ${financeFmt(x.budget,'TRY')}</b></div>`)}</article></div></section>`}
function financeUpcomingInstallments(days=30){const end=new Date(today()+'T00:00:00');end.setDate(end.getDate()+days);return state.financeFinancingInstallments.filter(i=>!['paid','cancelled'].includes(i.status||'pending')&&i.due_date&&new Date(i.due_date+'T00:00:00')<=end).sort((a,b)=>(a.due_date||'').localeCompare(b.due_date||''))}
function financeDashboardBudgetStatus(){const b=financeBudgetSummary();const percent=b.total>0?Math.round((b.actual/b.total)*100):0;return {...b,percent}}
function financeDashboardCards(){const s=financeAccountSummary(),tx=financeTransactionSummary(),fn=financeFinancingPlansSummary(),budget=financeDashboardBudgetStatus();return {s,tx,fn,budget}}
function financeDashboardInstallmentRow(i){const plan=state.financeFinancingPlans.find(p=>p.id===i.financing_plan_id);const st=financeInstallmentEffectiveStatus(i);return `<div class="priority-item"><div><b>${esc(plan?.provider_name||'Finansman')} #${Number(i.installment_no||0)}</b><small>${esc(financeInstallmentStatusLabel(st))} · ${esc(i.due_date||'')} · ${financeFmt(i.amount,'TRY')}</small></div><button class="small secondary" onclick="financeFinancingInstallmentForm('${i.financing_plan_id}','${i.id}')">Aç</button></div>`}
function financeDashboardAttentionItems(cards,upcoming){const items=[];const overdue=upcoming.filter(i=>financeInstallmentEffectiveStatus(i)==='overdue');if(overdue.length)items.push(`Geciken ${overdue.length} finansman taksiti var.`);if(cards.budget.percent>=100)items.push('Bu ay bütçe limiti aşılmış görünüyor.');else if(cards.budget.percent>=70)items.push('Bu ay bütçe kullanım oranı %70 üzerinde.');if(cards.tx.net<0)items.push('Bu ay net nakit akışı negatif.');if(!items.length)items.push('Kritik finans uyarısı görünmüyor.');return items.map(x=>`<div class="pill inactive"><span>${esc(x)}</span></div>`).join('')}
function financeDashboardDetail(){const c=financeDashboardCards(),upcoming=financeUpcomingInstallments(30);return `<section id="finance_dashboard_panel" class="card"><div class="row"><div style="flex:1"><span class="eyebrow">Finance Dashboard</span><h2>Finans Dashboard</h2><p class="muted">Nakit, borç, bütçe ve yaklaşan finansman yükümlülükleri tek ekranda.</p></div><button onclick="page('finance')">Finans’a Git</button></div><div class="finance-dashboard-grid"><div class="kpi"><span>Toplam Varlık</span><b class="pos">${fmt(c.s.assets)}</b></div><div class="kpi"><span>Toplam Borç</span><b class="neg">${fmt(c.s.liabilities+c.fn.remaining)}</b><small>Hesap borçları + kalan finansman</small></div><div class="kpi"><span>Net Durum</span><b class="${(c.s.net-c.fn.remaining)>=0?'pos':'neg'}">${fmt(c.s.net-c.fn.remaining)}</b></div><div class="kpi"><span>Bu Ay Net Akış</span><b class="${c.tx.net>=0?'pos':'neg'}">${fmt(c.tx.net)}</b></div><div class="kpi"><span>Bütçe Kullanımı</span><b class="${c.budget.percent>=100?'neg':'pos'}">%${c.budget.percent}</b><small>${fmt(c.budget.actual)} / ${fmt(c.budget.total)}</small></div><div class="kpi"><span>Kalan Finansman</span><b class="neg">${fmt(c.fn.remaining)}</b><small>Ödenen: ${fmt(c.fn.paid)}</small></div></div><div class="split"><div><h3>Yaklaşan Taksitler</h3>${upcoming.slice(0,5).map(financeDashboardInstallmentRow).join('')||'<div class="empty compact">30 gün içinde taksit yok.</div>'}</div><div><h3>Dikkat Edilecekler</h3>${financeDashboardAttentionItems(c,upcoming)}</div></div></section>`}
...
function exportData(){const blob=new Blob([JSON.stringify(state,null,2)],{type:'application/json'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='momentum-hub-v7-yedek.json';a.click()}

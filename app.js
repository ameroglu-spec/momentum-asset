...
let state={homes:[],cars:[],entries:[],definitions:[],documents:[],financeAccounts:[],financeTransactions:[],financeError:null,financeTransactionsError:null};
...
async function load(){const [h,c,e,d,doc,fa,ft]=await Promise.all([sb.from('homes').select('*').order('created_at'),sb.from('cars').select('*').order('created_at'),sb.from('entries').select('*').order('date',{ascending:false}),sb.from('definitions').select('*').order('sort_order').order('name'),sb.from('documents').select('*').order('created_at',{ascending:false}),sb.from('finance_accounts').select('*').eq('user_id',user.id).order('created_at'),sb.from('finance_transactions').select('*').eq('user_id',user.id).order('transaction_date',{ascending:false})]);if(h.error||c.error||e.error||d.error){toast('Supabase SQL eksik olabilir. V5 SQL kodunu çalıştır.');return}state.homes=h.data||[];state.cars=c.data||[];state.entries=e.data||[];state.definitions=d.data||[];state.documents=doc.data||[];state.financeError=fa.error?fa.error.message:null;state.financeAccounts=fa.error?[]:(fa.data||[]);state.financeTransactionsError=ft.error?ft.error.message:null;state.financeTransactions=ft.error?[]:(ft.data||[]);await ensureDefaults();page(currentPage||'dashboard')}
...
function finance(){
  const s=financeAccountSummary(),tx=financeTransactionSummary();const inactive=state.financeAccounts.filter(a=>a.is_active===false).length;
  $('content').innerHTML=`<div class="finance-shell"><section class="finance-hero card"><div><span class="eyebrow">Momentum Finance</span><h2>Finansal Durum</h2><p class="muted">Hesaplarını ve gelir/gider hareketlerini tek ekranda yönet.</p></div><div class="row"><button onclick="financeAccountForm()">+ Hesap Ekle</button><button onclick="financeTransactionForm()">+ Hareket Ekle</button></div></section><section class="finance-tabs"><button onclick="document.getElementById('finance_accounts_panel').scrollIntoView({behavior:'smooth'})">Hesaplar</button><button class="secondary" onclick="document.getElementById('finance_transactions_panel').scrollIntoView({behavior:'smooth'})">Hareketler</button></section><section class="finance-kpis"><div class="kpi"><span>Aktif Hesap</span><b>${s.count}</b><small>${inactive?inactive+' pasif hesap':''}</small></div><div class="kpi"><span>Toplam Nakit / Varlık</span><b class="pos">${fmt(s.assets)}</b><small>Sadece TRY hesaplar</small></div><div class="kpi"><span>Toplam Borç</span><b class="neg">${fmt(s.liabilities)}</b><small>Sadece TRY borçlar</small></div><div class="kpi"><span>Net Hesap Durumu</span><b class="${s.net>=0?'pos':'neg'}">${fmt(s.net)}</b><small>${s.otherCurrencies.length?'Döviz ayrı: '+s.otherCurrencies.join(', '):'Varlık - Borç'}</small></div></section><section class="finance-kpis"><div class="kpi"><span>Bu Ay Gelir</span><b class="pos">${fmt(tx.income)}</b><small>Tamamlanan TRY gelirler</small></div><div class="kpi"><span>Bu Ay Gider</span><b class="neg">${fmt(tx.expense)}</b><small>Tamamlanan TRY giderler</small></div><div class="kpi"><span>Net Akış</span><b class="${tx.net>=0?'pos':'neg'}">${fmt(tx.net)}</b><small>Gelir - gider</small></div><div class="kpi"><span>Hareket</span><b>${state.financeTransactions.length}</b><small>${tx.otherCurrencies.length?'Döviz ayrı: '+tx.otherCurrencies.join(', '):'Toplam kayıt'}</small></div></section>${state.financeError?`<section class="card danger"><b>Finance verisi yüklenemedi:</b> ${esc(state.financeError)}</section>`:''}${state.financeTransactionsError?`<section class="card danger"><b>Finance hareketleri yüklenemedi:</b> ${esc(state.financeTransactionsError)}</section>`:''}<section id="finance_accounts_panel"><div class="row"><h2 style="flex:1">Hesaplar</h2><button onclick="financeAccountForm()">+ Hesap Ekle</button></div><div class="finance-grid">${s.active.map(financeAccountCard).join('')||'<div class="card empty">Henüz finans hesabı yok. İlk nakit, banka veya kredi kartı hesabını ekle.</div>'}</div></section>${inactive?`<section class="card"><h2>Pasif Hesaplar</h2><div class="finance-passive-list">${state.financeAccounts.filter(a=>a.is_active===false).map(financeAccountMini).join('')}</div></section>`:''}<section id="finance_transactions_panel" class="card"><div class="row"><div style="flex:1"><h2>Hareketler</h2><p class="muted">Gelir/gider kayıtları hesaplara bağlanır. Otomatik bakiye motoru Sprint 2A.3 kapsamındadır.</p></div><button onclick="financeTransactionForm()">+ Hareket Ekle</button></div>${financeTransactionsList()}</section></div>`;
}
function financeAccountCard(a){const type=financeAccountTypeLabel(a.account_type);return `<article class="finance-account-card ${a.is_asset===false?'liability':'asset'}"><div class="finance-account-top"><div><span class="finance-account-icon">${financeAccountIcon(a.account_type)}</span><h3>${esc(a.name)}</h3><p>${esc(type)} · ${esc(a.currency||'TRY')}</p></div><span class="badge ${a.is_asset===false?'bad':'ok'}">${a.is_asset===false?'Borç':'Varlık'}</span></div><div class="finance-account-balance"><span>Güncel Bakiye</span><b class="${a.is_asset===false?'neg':'pos'}">${financeFmt(a.current_balance,a.currency||'TRY')}</b></div><div class="finance-account-meta"><span>${esc(a.institution||'Kurum yok')}</span><span>${a.card_last4?'**** '+esc(a.card_last4):esc(maskFinanceText(a.iban)||'')}</span></div><p class="muted">${esc(a.description||'')}</p><div class="asset-actions-v6"><button onclick="financeAccountForm('${a.id}')">Düzenle</button><button class="secondary" onclick="archiveFinanceAccount('${a.id}',false)">Pasife Al</button></div></article>`}
